...
void TextureSplatting::onPluginLoad() {
    ...
    QString fs_src =
      ...
      "out vec4 fragColor;"
      "void main() {"
      "vec4 noise = texture(sampler2, vtexCoord);"
      "fragColor = mix(texture(sampler0, vtexCoord),"
      "                texture(sampler1, vtexCoord), noise); }";
    ...
    loadTexture(GL_TEXTURE2, textureId2);
}

void loadTexture(GLuint tex, GLuint textureId) {
    g.glActiveTexture(tex);
	QString filename = QFileDialog::getOpenFileName(0, "Open Image", "/assig/grau-g/Textures", "Image file (*.png *.jpg)");	
	g.makeCurrent();
	QImage img(filename);	
	QImage im = img.convertToFormat(QImage::Format_ARGB32).rgbSwapped().mirrored();
	g.glGenTextures(1, &textureId);
	g.glBindTexture(GL_TEXTURE_2D, textureId);
	g.glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, im.width(), im.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, im.bits());
	g.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	g.glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	g.glBindTexture(GL_TEXTURE_2D, 0);
}
